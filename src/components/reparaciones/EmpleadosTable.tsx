...
export default function EmpleadosTable() {
  ...
  const fetchEmpleados = useCallback(async () => {
    ...
    try {
      ...
      let query = supabase
        .from('usuarios')
        .select('id_usuario, nombre, email, rol, descuento_maximo', { count: 'exact' })
  ...
  const handleSaveEmpleado = async (data: EmpleadoFormData, id_usuario: string | null) => {
    ...
    try {
      ...
      const finalData = {
        ...dataToSave,
        rol: dataToSave.rol || 'Tecnico',
        descuento_maximo: dataToSave.descuento_maximo ?? 0,
      }

      if (id_usuario) {
        // Update user in public.usuarios
        ...
        const { error: dbError } = await supabase
          .from('usuarios')
          .insert([{ ...finalData, empresa_id: empresaId, auth_uid: auth_uid }])
          ...
          {totalEmpleados > 0 ? (
            <>
              ...
              <span className='font-medium'>
                {Math.min(currentPage * itemsPerPage, totalEmpleados)}
              </span>{' '}
              de <span className='font-medium'>{totalEmpleados}</span> resultados
            </>
          ) : (
            'No hay resultados'
          )}
